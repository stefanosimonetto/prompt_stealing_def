...
# a single data point for the "Full" scenario in Table 6 of the paper.
...
OUTPUT_DIR="results/finetuning_${STYLE}_${DATASET_NAME}_full_${RUN_ID}"

echo -e "${BLUE}--- Running LoRA Fine-tuning (Full Scenario) for style: $STYLE on dataset: $DATASET_NAME ---${NC}"
echo -e "${BLUE}--- Output directory: $OUTPUT_DIR ---${NC}"

# --- Step 1: Run the LoRA fine-tuning process ---
...
echo -e "\n${BLUE}---${NC}"
echo -e "${GREEN}✅ Successfully completed fine-tuning and evaluation for style: '$STYLE' on '$DATASET_NAME' (Full Scenario)${NC}"
